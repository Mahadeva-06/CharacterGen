...
class TuneAVideoPipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image_pil, device, num_images_per_prompt, do_classifier_free_guidance, img_proj=None):
        ...
        # Ensure all tensors are on CPU before normalization
        clip_image_mean = clip_image_mean.to('cpu')
        clip_image_std = clip_image_std.to('cpu')
        imgs_in_proc = imgs_in_proc.to('cpu')
        ...
        if img_proj is None:
            ...
            image_embeds = self.image_encoder(imgs_in_proc, output_hidden_states=True).hidden_states[-2]
        ...
        image_latents = self.vae.encode((image_pil* 2.0 - 1.0).to('cpu')).latent_dist.mode() * self.vae.config.scaling_factor
    ...
    def prepare_latents(self, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None):
        shape = (int(batch_size), int(num_channels_latents), int(video_length), int(height // self.vae_scale_factor), int(width // self.vae_scale_factor))
