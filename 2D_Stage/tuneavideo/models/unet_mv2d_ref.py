...
from peft.tuners.lora import LoraLayer as AttentionProcessor
...
from tuneavideo.models.unet_mv2d_condition import DIFFUSERS_CACHE
...
@dataclass
class UNetMV2DRefOutput:
    sample: torch.FloatTensor = None

__version__ = "0.19.0.dev0"

class UNetMV2DRefModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        timestep_input_dim: Optional[int] = None,
    ):
        super().__init__()
    ...
    # ...existing code...
        print(f"[DEBUG] UNetMV2DRefModel: time_embedding in_channels={timestep_input_dim}, time_embed_dim={time_embed_dim}")
        ...
        if class_embed_type is None and num_class_embeds is not None:
            ...
            print(f"[DEBUG] UNetMV2DRefModel: class_embedding (timestep) in_channels={timestep_input_dim}, time_embed_dim={time_embed_dim}")
            self.class_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim, act_fn=act_fn, sample_proj_bias=True)
            ...
            print(f"[DEBUG] UNetMV2DRefModel: class_embedding (projection) in_channels={projection_class_embeddings_input_dim}, time_embed_dim={time_embed_dim}")
            self.class_embedding = TimestepEmbedding(projection_class_embeddings_input_dim, time_embed_dim, sample_proj_bias=True)
        ...
        if addition_embed_type == "text":
            ...
            print(f"[DEBUG] UNetMV2DRefModel: add_embedding (text_time) in_channels={projection_class_embeddings_input_dim}, time_embed_dim={time_embed_dim}")
            self.add_embedding = TimestepEmbedding(projection_class_embeddings_input_dim, time_embed_dim, sample_proj_bias=True)
        ...
        # All LoRACompatibleLinear and Identity logic removed for PEFT-only codebase
    ...
    def from_pretrained_2d(
        ...
        # local_files_only = kwargs.pop("local_files_only", HF_HUB_OFFLINE)  # Not available, comment out
        ...
        config, unused_kwargs, commit_hash = cls.load_config(
            ...
            # resume_download=resume_download,  # Removed: not supported by _get_model_file
            proxies=proxies,
        ...
        if from_flax:
            ...
            try:
                ...
                if lora_config is not None and get_peft_model is not None:
                    model = get_peft_model(model, lora_config)
                lora_weights_path = kwargs.pop("lora_weights_path", None)
                if lora_weights_path is not None and hasattr(model, "load_adapter"):
                    model.load_adapter(lora_weights_path)
            except Exception:
                pass
            state_dict = load_state_dict(model_file)
            # model._convert_deprecated_attention_blocks(state_dict)  # Removed: method does not exist in this class
