...
# Define default cache directory for diffusers
...
class UNetMV2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def from_pretrained_2d(
        ...
        print("    [UNetMV2DConditionModel] from_pretrained_2d: start")
        r"""
        ...
        print(f"    [UNetMV2DConditionModel] from_pretrained_2d: cache_dir={cache_dir}")
        ...
        # load config
        print(f"    [UNetMV2DConditionModel] from_pretrained_2d: loading config from {config_path}")
        ...
        # load model
        print("    [UNetMV2DConditionModel] from_pretrained_2d: loading model file")
        model_file = None
        if from_flax:
            raise NotImplementedError
        else:
            ...
            print("    [UNetMV2DConditionModel] from_pretrained_2d: creating model from config")
            model = cls.from_config(config, **unused_kwargs)
            # --- PEFT LoRA integration ---
            # If LoRA/PEFT is desired, wrap the model with PEFT's LoraModel here.
            # This assumes PEFT is installed and available.
            try:
                from peft import get_peft_model, LoraConfig
                # Example: you may want to pass a config or load from a checkpoint
                # Here we use a default config for demonstration; adjust as needed.
                # You can also add a parameter to this function to pass a LoRA config or checkpoint.
                lora_config = kwargs.pop("lora_config", None)
                if lora_config is not None:
                    model = get_peft_model(model, lora_config)
                # Optionally, load LoRA weights if provided
                lora_weights_path = kwargs.pop("lora_weights_path", None)
                if lora_weights_path is not None:
                    model.load_adapter(lora_weights_path)
            except ImportError:
                pass  # PEFT not installed, skip
            print(f"    [UNetMV2DConditionModel] from_pretrained_2d: loading state_dict from {model_file}")
            try:
                print("    [UNetMV2DConditionModel] from_pretrained_2d: before load_state_dict")
                state_dict = load_state_dict(model_file, map_location="cpu")
                print("    [UNetMV2DConditionModel] from_pretrained_2d: after load_state_dict")
            except Exception as e:
                print(f"    [UNetMV2DConditionModel] from_pretrained_2d: Exception in load_state_dict: {e}")
                import traceback
                traceback.print_exc()
                raise
            # model._convert_deprecated_attention_blocks(state_dict)  # Removed: not needed with PEFT

            try:
                print(f"    [UNetMV2DConditionModel] from_pretrained_2d: state_dict loaded, keys: {list(state_dict.keys())[:10]} ... total: {len(state_dict)}")
                # print(f"    [UNetMV2DConditionModel] from_pretrained_2d: SKIPPING all model processing for isolation test.")
                return model
            except Exception as e:
                print(f"    [UNetMV2DConditionModel] from_pretrained_2d: Exception after load_state_dict: {e}")
                import traceback
                traceback.print_exc()
                raise
        ...
        print("    [UNetMV2DConditionModel] from_pretrained_2d: finalizing model dtype and config")
        ...
        model.eval()
        print("    [UNetMV2DConditionModel] from_pretrained_2d: done, returning model")
